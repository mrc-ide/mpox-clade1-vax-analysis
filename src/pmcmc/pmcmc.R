...
orderly_pars <- orderly2::orderly_parameters(region = "both",
                                             deterministic = TRUE,
                                             fit_by_age = TRUE,
                                             fit_KPs = TRUE,
...
orderly2::orderly_resource(
  c(paste("parameters", region, type, assumptions, "info.csv", sep = "/"),
    paste("parameters", region, type, assumptions, "proposal.csv", sep = "/")))
...
snapshots <- "2024-01-07"

raw_data <- readRDS("inputs/data.rds")
raw_data_linelist <- readRDS("inputs/data_linelist.rds")
raw_data_who_shiny <- readRDS("inputs/data_who_shiny.rds")
data <- get_data(raw_data, raw_data_linelist, raw_data_who_shiny, start_date,
                 end_date, fit_by_age, fit_KPs, region)

mcmc_pars <- create_mcmc_pars(region, deterministic, assumptions)
...
filter <- create_filter(data, deterministic, region, start_date, control)
...
pmcmc_results <- calc_Rt_fits(pmcmc_results, region)

saveRDS(pmcmc_results, "outputs/samples.rds")
saveRDS(data, "outputs/fitting_data.rds")
save_pars_inputs(pmcmc_results, mcmc_pars, region)
