...
orderly_pars <- 
  orderly2::orderly_parameters(region = "equateur",
                               use_both_fit = FALSE,
                               ...
                               vaccine_used = "mix", #"Lc16m8" # "MVA-BN" #"mix"
                               uptake_realised = 1,
                               t_ve = 28, 
...
# Reduce R0 in sex workers as determined by R0_SW_reduction
if(use_both_fit==TRUE){
samples$pars[paste0("R0_sw_st<",region,">"),]<-samples$pars[paste0("R0_sw_st<",region,">"),]*(1-R0_SW_reduction)
}else{
...
vax_pars <- get_vaccine_scenario_parameters(
  ...
  t_ve = t_ve,
  t1=t1, 
  vaccine_used = vaccine_used, # "mix"  #"Lc16m8"  # "MVA-BN",
